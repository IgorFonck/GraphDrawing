...
label_dense = "f(n) = n * {:f}".format(parametros[0])
plt.plot(tamanhos_aproximados, aproximados, label=label_dense, color="#7373ff", linestyle="dotted")
...
tamanhos_aproximados = range(n_sizes[-1] * range_multiplier)
aproximados = [funcao_linear(x, *parametros) for x in tamanhos_aproximados]
label_sparse = "g(n) = n * {:f}".format(parametros[0])
plt.plot(tamanhos_aproximados, aproximados, label=label_sparse, color="#ff7373", linestyle="dotted")
